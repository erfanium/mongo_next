...
async function main(args) {
  args = yargs(args)
    ...
    .option("f", {
      ...
      describe:
        "The test name filter, if none provided all test logs will be shown",
      type: "string",
    })
    .option("v", {
      alias: "verbose",
  ...
  if (!existsSync("xunit.xml")) {
    console.error(
      "xunit.xml file not found, required for db log test filtering.",
    );
    process.exit(1);
  }
...
function interpolateMsg(log) {
  ...
  for (const key in log.attr) {
    if (Reflect.has(log.attr, key)) {
      log.msg = log.msg.split(`{${key}}`).join(
        `${JSON.stringify(log.attr[key])}`,
      );
      delete log.attr[key];
    }
  }
...
function friendlyDate(log) {
  ...
  try {
    ...
    if (warnings) {
      console.error(`Cannot translate date time of ${JSON.stringify(log)}`);
    }
  }
}
    ...
    if (suite.testcase) {
      for (const test of suite.testcase) {
        ...
        if (fullName.toLowerCase().includes(testFilter.toLowerCase())) {
          if (test.$.start === "0") {
            if (warnings) {
              console.error(`Warning: ${fullName} was skipped, theres no logs`);
            }
            continue;
          }
          tests.push({
