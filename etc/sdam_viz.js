...
const { now, calculateDurationInMs, arrayStrictEqual, errorStrictEqual } =
  require("../src/utils");
...
const argv = require("yargs")
  ...
  .describe(
    "writeWorkloadInterval",
    "Time interval between write workload write attempts",
  )
  .describe(
    "writeWorkloadSampleSize",
    "Sample size between status display for write workload",
  )
...
function diff(lhs, rhs, fields, comparator) {
  return fields.reduce((diff, field) => {
    ...
    if (!comparator(lhs[field], rhs[field])) {
      diff.push(
        `  ${field}: ${chalk.green(`${util.inspect(lhs[field])}`)} => ${
          chalk.green(
...
function visualizeMonitoringEvents(client) {
  ...
  client.on("serverHeartbeatSucceeded", (event) =>
    print(
      ...
      } host: '${event.connectionId}' ${chalk.gray(`(${event.duration} ms)`)}`,
    ));
  ...
  client.on("serverOpening", (event) => {
    print(
      `${chalk.cyan("server")} [${event.address}] ${
        chalk.bold("opening")
      } in topology#${event.topologyId}`,
    );
  });
  ...
  client.on("serverDescriptionChanged", (event) => {
    ...
    console.error(
      serverDescriptionDiff(event.previousDescription, event.newDescription),
    );
  });
  ...
  client.on("topologyDescriptionChanged", (event) => {
    const diff = topologyDescriptionDiff(
      event.previousDescription,
      event.newDescription,
    );
    if (diff !== "") {
      print(
        `${chalk.magenta("topology")} [topology#${event.topologyId}] changed:`,
      );
      console.error(diff);
    }
  });
...
async function scheduleWorkload(client) {
  ...
  try {
    ...
    print(
      `${chalk.yellow(`workload#${currentWorkload}`)} find completed: ${
    ...
    print(
      `${
        chalk.yellow(`workload#${currentWorkload}`)
...
async function scheduleWriteWorkload(client) {
  ...
  try {
    ...
    print(
      `${
        chalk.yellow(`workload#${currentWriteWorkload}`)
